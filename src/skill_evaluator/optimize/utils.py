...
Extracted from ai-dev-kit/.test/src/skill_test/optimize/utils.py

Provides path resolution, token counting,
...
def find_repo_root() -> Path:
    ...
    while current != current.parent:
        if (current / ".test" / "src").exists():
            return current
        if (current / "src" / "skill_evaluator").exists() and current.name == ".test":
